function createACAbilityRequest(acNumber) {
    const header = Buffer.from([0x55, 0x55, 0x55, 0xAA]);
    const address = Buffer.from([0x90, 0xB0]); // Note the different address for extended messages
    const messageId = Buffer.from([0x01]);
    const messageType = Buffer.from([0x1F]);
    const dataLength = Buffer.from([0x00, 0x03]);
    const subMessage = Buffer.from([0xFF, 0x11, acNumber]);
...
module.exports = createACAbilityRequest;
